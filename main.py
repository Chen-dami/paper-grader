...
def main():
    ...
    print(f"阅卷系统 | {provider} | {mode} | {q_count}题 | {total_papers}份\n")
    ...
    for cls_name in sorted(paper_files.keys()):
        ...
        print(f"  {cls_label}: {len(files)}人  ", end="", flush=True)
        ...
        for i, f in enumerate(files):
            ...
            if (i + 1) % step == 0 or i == len(files) - 1:
                pct = (i + 1) / len(files)
                bar = "|" + "=" * int(pct * 15) + ">" + " " * (15 - int(pct * 15)) + "|"
                elapsed = time.time() - t_cls
                per_paper = elapsed / (i + 1)
                print(f"\r  {cls_label}: {len(files)}人 {bar} {i+1}/{len(files)} | {per_paper:.1f}s/份", end="")

        # 班级汇总
        if class_results:
            cls_out = os.path.join("output", cls_label)
            class_summary_report(class_results, rubric, cls_out, cls_label)
            avg = sum(r["total_score"] for r in class_results) / len(class_results)
            print(f"\r  {cls_label}: {len(files)}人 | 平均{avg:.0f}分 | 汇总表已生成  ", flush=True)

            # 班级内查重
            if args.check and len(files) >= 2:
                from src.plagiarism import run as check_run
                cls_papers_dir = os.path.join(papers_dir, cls_label) if cls_label != "." else papers_dir
                check_run(cls_papers_dir, cls_out)
