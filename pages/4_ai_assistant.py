...
GRADING_SYSTEM_PROMPT = """你是阅卷系统的 AI 阅卷控制助手。你可以帮教师调整本次阅卷的评分策略和模型设置。

## 可用操作

### 视觉策略（vision_strategy）
...
## 规则
- 用中文回复，简洁直接
- 当用户要求调整上述设置时，在回复末尾输出变更块：

```action
{"type":"grading_override","summary":"一句话","overrides":{"vision_strategy":"free_vision","mode":"strict"}}
```

- overrides 只包含要修改的项，不需要全部列出
- 若用户只是咨询或聊天，不输出 action 块
- 例："帮我把视觉改成免费的" → {"type":"grading_override","summary":"视觉策略切换为免费视觉","overrides":{"vision_strategy":"free_vision"}}
- 例："严格点评分" → {"type":"grading_override","summary":"切换为严格模式","overrides":{"mode":"strict"}}
- 例："用纯文字，放松点评" → {"type":"grading_override","summary":"纯文字+宽松模式","overrides":{"vision_strategy":"text_only","mode":"relaxed"}}"""
...
def call_llm(messages, stream=False):
    ...
    # 阅卷控制模式：注入阅卷控制系统提示词
    if st.session_state.ai_mode == "grading":
        from src.grading_overrides import get_overrides, describe_overrides
        ctx = f"## 当前覆盖设置\n{describe_overrides()}"
        sys_prompt = GRADING_SYSTEM_PROMPT + "\n\n" + ctx
        full_msgs.append({"role": "system", "content": sys_prompt})
...
def apply_grading_override(overrides: dict):
    """应用阅卷控制覆盖（立即生效，无需确认）"""
    from src.grading_overrides import set_override
    results = []
    for key, value in overrides.items():
        if set_override(key, value):
            from src.grading_overrides import AVAILABLE_OVERRIDES
            info = AVAILABLE_OVERRIDES.get(key, {})
            label = info.get("option_labels", {}).get(value, value)
            results.append(f"{info.get('label', key)} → {label}")
        else:
            results.append(f"{key}={value}（无效，已忽略）")
    return results
...
with col_main:
    ...
    with ctl2:
        # 模式切换
        mode_opts = {"chat": "自由对话", "grading": "阅卷控制", "config": "评分配置"}
    ...
    elif st.session_state.ai_mode == "grading":
        from src.grading_overrides import get_overrides, describe_overrides, clear_overrides
        ov = describe_overrides()
        c1, c2 = st.columns([3, 1])
        with c1:
            st.info(f"阅卷控制模式：可调整视觉策略、评分模式等。当前：{ov}")
        with c2:
            if "grading_overrides" in st.session_state and st.session_state.grading_overrides:
                if st.button("↩ 恢复默认", key="reset_grading_ov"):
                    clear_overrides()
                    st.rerun()
    ...
    for i, msg in enumerate(msgs):
        with st.chat_message(msg["role"]):
            ...
            if action and not msg.get("action_done"):
                ...
                if atype in ("rubric", "config", "both"):
                    # 评分配置：需要确认
                    with st.container(border=True):
                        ...
                        with cb:
                            if st.button("↩ 放弃", key=f"cx_{i}"):
                                st.session_state.ai_threads[current_thread()]["messages"][i]["action_done"] = True
                                st.rerun()
                elif atype == "grading_override":
                    # 阅卷控制：已自动应用（在消息创建时已生效）
                    pass  # action_result 会在下面显示
    ...
    if prompt:
        ...
        # 解析动作：config 和 grading 模式都可能有 action
        parse_modes = ["config", "grading"]
        clean, action = parse_action(raw_reply) if st.session_state.ai_mode in parse_modes else (raw_reply, None)
        tid = current_thread()

        # 阅卷控制：自动应用，无需确认
        action_done = False
        action_result = None
        if action and action.get("type") == "grading_override" and action.get("overrides"):
            action_result = apply_grading_override(action["overrides"])
            action_done = True  # 立即生效
